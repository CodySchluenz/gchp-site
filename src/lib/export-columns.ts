...
export function sherlynHeaders(season: number): string[] {
  return [
    'tNo', `${season} Applicant`, 'Address', 'Special Gift', 'adopted', 'Adopted by', 'Thanksgiving',
...
export function fullRow(r: ExportRow): (string | number | null)[] {
  ...
  // The four adopter detail fields are gated on r.adopted === 1, not just
  // shown raw — clearAdoption keeps stale field values by design (see
  // db.ts), and this backup must never present a not-currently-adopted
  // family's old adopter details as current.
  const adopterDetails: (string | number | null)[] = r.adopted === 1
    ? [r.adopter_name, r.adopter_contact, r.adopter_phone, r.adopter_address]
    : ['', '', '', ''];
  return [
    ...
    yes(r.adopted_last_year), yes(r.adopted), ...adopterDetails,
    r.bed_choice, r.bed_size ?? '', income, r.employment_summary,
